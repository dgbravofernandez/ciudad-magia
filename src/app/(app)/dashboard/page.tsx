...
export default async function DashboardPage() {
  ...
  const [playersRes, sessionsRes, paymentsRes, expensesRes] = await Promise.all([
    ...
    supabase.from('expenses').select('amount').eq('club_id', clubId).gte('date', new Date(new Date().getFullYear(), new Date().getMonth(), 1).toISOString()),
  ])

  const totalPlayers = playersRes.data?.length ?? 0
  const activePlayers = playersRes.data?.filter(p => p.status === 'active').length ?? 0
  const sessionsThisMonth = sessionsRes.data?.length ?? 0
  const revenueThisMonth = paymentsRes.data?.reduce((s, p) => s + Number(p.amount), 0) ?? 0
  const expensesThisMonth = expensesRes.data?.reduce((s, e) => s + Number(e.amount), 0) ?? 0
